...
# docker build --target manager -t dfy007/admission-webhook-example:v1.3 -f Dockerfile .

FROM alpine:3.9.2 as tls
COPY --from=builder /workspace/tls-manager .
ENTRYPOINT ["/tls-manager"]
# docker build --target tls -t dfy007/admission-registry-tls:v1.3 -f Dockerfile .